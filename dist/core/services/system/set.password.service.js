...
import { passwordSessionModel, userCredentialsModel, } from '../../../database/models/index.js';
import { Template } from '../../../email/templates/template.js';
export class SetPasswordService {
    static both = async (req) => {
        try {
            ...
            if (passwordSession.typeOfSession === 'newUserPassword') {
                ...
                const setUserPassword = await userCredentialsModel.updateOne({ userId: userId }, { $set: { password: hashedPassword } });
                if (setUserPassword) {
                ...
                const hashedPassword = await bcrypt.hash(newPassword, 8);
                await userCredentialsModel.updateOne({ userId: userId }, { $set: { password: hashedPassword } });
                await passwordSessionModel.deleteOne({ _id: magicToken });
                return Promise.resolve({
                    code: 200,
                    success: true,
