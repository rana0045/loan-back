...
export class ForgotPasswordService {
  static update = async (req: Request) => {
    try {
      ...
      const passwordSessionId = await PasswordSessionHelper.findID(
        emailExist?.data?.userId as Types.ObjectId,
        'newForgotPassword',
      );
      if (!passwordSessionId) {
        await PasswordSessionHelper.createPasswordSession(
          emailExist?.data?.userId,
          'newForgotPassword',
        );
