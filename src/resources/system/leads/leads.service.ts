...
export namespace LeadsService {
  export const Leads = async (req: Request) => {
    try {
      ...
      if (req.query.type === 'DM') {
        const FindLeads = await LeadModel.ModuleFourModel.aggregate([
          ...
          {
            $sort: {
              createdAt: Filters.sorting as any,
            },
          },
          {
            $group: {
      ...
      if (req.query.type === 'BF') {
        const FindLeads =
          await LeadModel.BusinessFundingBusinessDetailsModel.aggregate([
            {
              $match: {
                $or: [
                  {
